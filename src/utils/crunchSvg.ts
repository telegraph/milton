...
import { path2js, js2path } from "./lib/path";
import { cleanupOutData } from "./lib/tools";
import { convertShapesToPath } from "./lib/shapeToPath";
...
let roundData: { (arg0: any[]): any[]; (data: any): any };
let precision: number | boolean | undefined;
let error: number;
let arcThreshold: number;
let arcTolerance: number;
let hasMarkerMid: boolean;
let hasStrokeLinecap: string | boolean | null;

export function crunchSvg(svg: SVGElement) {
  for (const node of Array.from(svg.querySelectorAll("*")) as SVGElement[]) {
 ...
 * @author Kir Belevich
 */
export function optimiseElement(item: SVGElement) {
